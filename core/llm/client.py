...
class GeminiClient(LLMClient):
    """
    Google Gemini LLM client using the OpenAI-compatible endpoint.

    Uses Gemini's native API with an OpenAI-compatible interface, so no
    additional dependencies are required beyond what's already in use.
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        Generate text using the Gemini API (OpenAI-compatible format).
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if e.code in (429, 500, 502, 503) and attempt < max_retries:
                    ...
                    logger.warning(
                        f"Gemini rate limited ({e.code}), retrying in {delay:.1f}s "
                ...
                logger.error(
                    f"Gemini HTTP error: {e.code} - {error_body}",
        ...
        logger.error(
            f"All {max_retries + 1} Gemini attempts failed",
            extra={"model": self.config.model}
        )
        return LLMResponse(
            content="",
...
def get_llm_client(
    ...
    Auto-detects provider based on environment variables (in priority order):
        1. GEMINI_API_KEY → GeminiClient (direct Google API, free tier)
        2. (fallback)     → OllamaClient (local)
    ...
    if provider == "gemini":
        if config is None:
            config = LLMConfig(
                model=os.getenv("LLM_MODEL", "gemini-2.5-flash-lite"),
                base_url=GeminiClient._BASE_URL,
